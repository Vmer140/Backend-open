...
public class PropertyCommandService {
    ...
    /**
     * Resuelve la unidad del inquilino autenticado a partir de su email.
     * Se usa en GET /api/units/me: es el punto de entrada del TENANT al dashboard,
     * ya que GET /api/units?buildingId=... es admin-only.
     */
    public Unit getMyUnit(String email) {
        User user = userRepository.findByEmail(new Email(email))
                .orElseThrow(() -> new ResourceNotFoundException("Usuario no encontrado: " + email));
        return unitRepository.findByTenantUserId(user.getId())
                .orElseThrow(() -> new ResourceNotFoundException(
                        "No hay una unidad asignada al usuario " + email));
    }
