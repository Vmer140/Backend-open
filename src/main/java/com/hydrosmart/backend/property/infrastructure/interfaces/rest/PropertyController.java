...
public class PropertyController {
    ...
    /**
     * Devuelve la unidad del inquilino autenticado.
     * Es la puerta de entrada del TENANT al dashboard, ya que
     * GET /api/units?buildingId=... está restringido a BUILDING_ADMIN.
     */
    @GetMapping("/units/me")
    public ResponseEntity<ApiResponse<UnitResource>> getMyUnit(@AuthenticationPrincipal UserDetails userDetails) {
        Unit unit = propertyCommandService.getMyUnit(userDetails.getUsername());
        return ResponseEntity.ok(ApiResponse.ok(
                UnitResourceAssembler.toResource(unit, propertyCommandService.resolveTenantName(unit.getTenantUserId()))));
    }
    ...
    public ResponseEntity<ApiResponse<ConsumptionSummaryResource>> getUnitSummary(
        ...
        // Si no es admin, tiene que ser el inquilino ASIGNADO a esta unidad.
        // El check anterior (unit.getTenantUserId() == null) dejaba que
        // cualquier tenant leyera el consumo de cualquier unidad ocupada.
        if (!isAdmin) {
            Unit myUnit = propertyCommandService.getMyUnit(userDetails.getUsername());
            if (!myUnit.getId().equals(unit.getId())) {
